...
internal/
  ...
  store/                    # SQLite 存取层（db.go 含内联 schema）
    toolfiles.go            # tools/ 目录文件读写（MCP→JSON, Skill→Markdown, Prompt→Markdown）
    messages.go             # 对话消息记录
...
tools/                      # 项目级工具定义 + 提示词
  mcp/                      # MCP 服务配置（.json 文件，启动时同步到 DB）
  skill/                    # Skill 配置（.md 文件，启动时同步到 DB）
  prompt/                   # 提示词模板（.md 文件，运行时直接读取，不经过 DB）
...
- **Prompt**：`tools/prompt/{name}.md`（纯 Markdown，支持 Go template 变量）
- 下划线开头的文件（如 `_example.json`）为示例，启动时自动跳过
- MCP/Skill：启动时扫描并同步到 DB；管理后台保存/删除时同步到文件
- Prompt：运行时直接从文件读取（`store.LoadPrompt(name)`），不经过 DB
...
### 提示词模板
提示词保存在 `tools/prompt/*.md`，支持 Go `text/template` 语法：
- `system.md`：LLM 系统提示词（无变量）
- `intent.md`：意图识别（变量：`{{.Message}}`、`{{.ProjectList}}`）
- `issue.md`：问题排查（变量：`{{.Message}}`、`{{.ProjectName}}`、`{{.Repos}}`）
- `requirement.md`：需求编写（变量：`{{.Message}}`、`{{.ProjectName}}`、`{{.Repos}}`、`{{.DocContent}}`）

修改文件后立即生效，无需重启。

### 数据库 Schema
Schema 内联在 `store/db.go` 的 `migrate()` 中（`CREATE TABLE IF NOT EXISTS`，幂等），不依赖外部文件。
启动时自动迁移旧列（`repo_path` → `repos`）。提示词已从 DB 迁移到文件，`prompt_templates` 表在启动时自动清理。
