...
提示词：tools/prompt/*.md 文件（直接读取，不经过 DB）
```
...
tools/
  ...
  prompt/
    system.md           # LLM 系统提示词
    intent.md           # 意图识别提示词（支持 {{.Message}}、{{.ProjectList}} 变量）
    issue.md            # 问题排查提示词（支持 {{.Message}}、{{.ProjectName}}、{{.Repos}} 变量）
    requirement.md      # 需求编写提示词（支持 {{.Message}}、{{.ProjectName}}、{{.Repos}}、{{.DocContent}} 变量）
```

MCP / Skill 可通过管理后台「工具配置」页面在线管理，保存时自动同步到文件。
提示词直接编辑 `tools/prompt/*.md` 文件即可。
...
| `/tools` | MCP / Skill 工具配置（提示词已迁移到 `tools/prompt/*.md` 文件） |
| `/harness` | 执行策略（自动化开关） |
