...
func ListRoutes(enabledOnly bool) ([]*model.ProjectRoute, error) {
	query := `SELECT id, name, keywords, repo_path, remote_url, doc_source,
...
func GetRoute(id string) (*model.ProjectRoute, error) {
	row := DB.QueryRow(`SELECT id, name, keywords, repo_path, remote_url, doc_source,
		mcp_list, skill_list, priority, enabled, created_at, updated_at
...
func CreateRoute(r *model.ProjectRoute) error {
	...
	_, err := DB.Exec(`
		...
		(id, name, keywords, repo_path, remote_url, doc_source, mcp_list, skill_list, priority, enabled, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, r.ID, r.Name, string(keywords), r.RepoPath, r.RemoteURL, r.DocSource,
...
func UpdateRoute(r *model.ProjectRoute) error {
	...
	_, err := DB.Exec(`
		UPDATE project_routes SET
			name=?, keywords=?, repo_path=?, remote_url=?, doc_source=?,
			mcp_list=?, skill_list=?, priority=?, enabled=?, updated_at=?
		WHERE id=?
	`, r.Name, string(keywords), r.RepoPath, r.RemoteURL, r.DocSource,
...
func scanRoute(row scanner) (*model.ProjectRoute, error) {
	...
	var keywords, mcpList, skillList string
	var enabled int
	err := row.Scan(
		&r.ID, &r.Name, &keywords, &r.RepoPath, &r.RemoteURL, &r.DocSource,
